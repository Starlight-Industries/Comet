...
pub fn set_panic_hook() {
    ...
    INIT.call_once(|| {
        ...
        std::panic::set_hook(Box::new(move |info| {
            let log_path = get_working_dir().unwrap_or_else(|_| {
                default_hook(info);
                std::process::exit(0);
            });
            if !log_path.exists() {
            ...
            let log_path = log_path.join("fatal-panic.log");

            // human_panic::print_msg::<PathBuf>(Some(log_path), &human_panic::Metadata::new("Comet", env!("CARGO_PKG_VERSION"))
            // .support("https://github.com/Comet-Engine/Comet/issues")
            // .authors("Comet community <https://github.com/Comet-Engine>")).unwrap();
...
"Comet had a problem and crashed. To help us diagnose the problem you can send us a crash report.

We have generated a report file at \"{}\". Submit an issue or email with the subject of \"Comet Crash Report\" and include the report as an attachment.
...
pub fn write_backtrace(trace: String,error: String, path: &Path) {
    let mut file = std::fs::File::create(path).expect("Failed to create backtrace file");
    writeln!(file, "{}", error).expect("Failed to write backtrace");
    writeln!(file, "{}", trace.sanitize_path()).expect("Failed to write backtrace");

    file.flush().expect("Failed to flush backtrace file");
    println!("Backtrace written to {}", path.display());
}
